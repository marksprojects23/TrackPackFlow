...
tpf_test_() ->
    {setup,
     fun() -> %this setup fun is run once befor the tests are run. If you want setup and teardown to run for each test, change {setup to {foreach
        ...
        meck:expect(data_service, store_package, fun(PackageLocation_map,Pid) -> worked end),
        meck:expect(data_service, update_location, fun(LocationCoord_map,Pid) -> worked end),
    ...
    [%This is the list of tests to be generated and run.
        ?_assertEqual({reply,worked,some_Db_PID},   % Putting package in facility
                            tpf_mock:handle_call({storing_package, #{package_id => "5d5ced29-c984-4d9d-9a0d-b77d4f53210b", location_id => "Warehouse13"}}, some_from_pid, some_Db_PID)
                            ),
        ...
        ?_assertEqual({noreply,some_Db_PID},        % Updating coords of a transit vehicle
                            tpf_mock:handle_cast({updating_location, #{location_id => "Truck2", latitude => 40.741895, longitude => -73.989308}}, some_Db_PID)
                                ),
        ...
        ?_assertEqual({reply,{fail,empty_key},some_Db_PID}, % Fail test
                            tpf_mock:handle_call({storing_package, ""}, some_from_pid, some_Db_PID)
                            )
    ]}.
